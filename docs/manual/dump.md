<!-- Adapted from the SPARTA manual (dump.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->

# dump command

# "dump image"_dump_image command
...
{image} args = discussed on "dump image"_dump_image doc page
...
Dump a snapshot of simulation quantities to one or more files every N timesteps in one of several styles.  The *image* style is the exception; it creates a JPG or PPM image file of the simulation configuration every N timesteps, as discussed on the "dump image"_dump_image doc page.
...
The *particle* and *grid* and *surf* styles create files in a simple text format that is self-explanatory when viewing a dump file.  Many of the OpenEdge post-processing tools, including Pizza.py.github.io/pizza, work with this format.
...
The specified filename determines how the dump file(s) is written. The default is to write one large text file, which is opened when the dump command is invoked and closed when an undump command is used or when OpenEdge exits.
...
The *p_name* and *p_name\[N\]* attributes allow custom per-particle vectors or arrays defined by some other command to be output.  The name should be replaced by the name of the attribute.  See "Section 6.17"_Section_howto for more discussion of custom attributes and command that define them.  For example, the "fix ambipolar"_fix_ambipolar command which defines the per-particle custom vector "ionambi" and custom array "velambi".
...
The *g_name* and *g_name\[N\]* attributes allow custom per-grid cell vectors or arrays defined by some other command to be output.  The name should be replaced by the name of the attribute.  See "Section 6.17"_Section_howto for more discussion of custom attributes and command that define them.  For example, the read_grid and "surf_react implicit"_surf_react_implicit commands can define per-grid attributes.  (The surf/react implicit command has not yet been released in public OpenEdge).
...
See Section 10 of the manual for information on how to add new compute and fix styles to OpenEdge to calculate per-grid quantities which could then be output into dump files.
...
See Section 10 of the manual for information on how to add new compute and fix styles to OpenEdge to calculate per-surf quantities which could then be output into dump files.

---

## Restrictions

To write gzipped dump files, you must compile OpenEdge with the -DOpenEdge_GZIP option - see the the build instructions section of the documentation.
...
The defaults for the image style are listed on the "dump image"_dump_image doc page.
