<!-- Adapted from the SPARTA manual (read_particles.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->
...
file = dump file to read snapshot from Nstep = timestep to read :ul
...
Read a snapshot of particles from a previously created dump file and add them to the simulation domain.  This is a means of reading in particles from a previous OpenEdge simulation or created as output by another code.  The create_particles, "fix emit/face"_fix_emit_face, and read_restart commands are alternate ways to generate particles for a simulation.

The dump file must be in the OpenEdge format created by the "dump particles"_dump command which is described on its doc page.
