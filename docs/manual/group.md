<!-- Adapted from the SPARTA manual (group.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->
...
Assign grid cells to grid groups or surface elements to surface groups.  In OpenEdge, a "grid group" is a collection of one or more grid cells.  A "surface group" is a collection of one or more surface elements (line segements in 2d, triangles in 3d).  Other commands take group IDs as arguments so that they act on a set of grid cells or surface elements.  For example, see the "compute grid"_compute_grid, compute surf, "fix ave/grid"_fix_ave_grid, fix ave/surf, "dump grid"_dump, or dump surf commands.
...
Note that this command assigns all flavors of child grid cells to groups, which includes unsplit, cut, split, and sub cells.  See Section 6.8 of the manual gives details of how OpenEdge defines child, unsplit, split, and sub cells.
...
dump, region, "compute grid"_compute_grid, compute surf
