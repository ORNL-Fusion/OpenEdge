<!-- Adapted from the SPARTA manual (global.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->
...
keyword = {fnum} or {nrho} or {vstream} or {temp} or {field} or {efield} or {bfield} or {surfs} or {surfgrid} or {surfmax} or {splitmax} or {surftally} or {gridcut} or {comm/sort} or {comm/style} or {weight} or {particle/reorder} or {mem/limit} or {pusher} or {boris_subcycles} or {boris_dump} or {boris_dump_every} or {boris_bad_dt_check} or {boris_bad_dt_limit}
  {fnum} value = ratio
  ...
  {efield} values = efstyle args
    efstyle = {none} or {particle} or {grid}
      {none} arg = none
      {particle} args = fixID Nfreq
        fixID = ID of fix efield/particle that computes per particle E-field components
        Nfreq = update field values every this many timesteps (0 = once at start)
      {grid} args = fixID Nfreq
        fixID = ID of fix efield/grid that computes per grid cell E-field components
        Nfreq = update field values every this many timesteps (0 = once at start)
  {bfield} values = bfstyle args
    bfstyle = {none} or {particle} or {grid}
      {none} arg = none
      {particle} args = fixID Nfreq
        fixID = ID of fix bfield/particle that computes per particle B-field components
        Nfreq = update field values every this many timesteps (0 = once at start)
      {grid} args = fixID Nfreq
        fixID = ID of fix bfield/grid that computes per grid cell B-field components
        Nfreq = update field values every this many timesteps (0 = once at start)
...
*pusher* values = one or more pusher keyword/value pairs
pusher keyword = *mode*, *plasma*, *subcycles*, *gca_switch*,
*boris_near*, *gca_integrator*, *dump*, *dump_every*,
*bad_dt_check*, *bad_dt_limit*, or *sheath*
*mode* value = *boris*, *hybrid*, or *gca*
*plasma* value = compute or fix ID providing plasma fields
*subcycles* value = positive integer
*gca_switch* value = positive GCA validity factor
*boris_near* value = non-negative distance
*gca_integrator* value = *rk2*, *rk4*, or *simple*
*dump* value = yes or no
*dump_every* value = positive integer
*bad_dt_check* value = yes or no
*bad_dt_limit* value = positive number
*sheath* value = *off*, *kick*, *spatial*, or *boundary*, followed by
optional {geom} ID, {mD_amu} mass, and {dmax} distance
*boris_subcycles* value = N
N = number of Boris substeps per particle move timestep (N > 0)
*boris_dump* value = yes or no
yes/no = print Boris E/B debug line (rank 0, particle index 0)
*boris_dump_every* value = N
N = print Boris debug every N timesteps (N > 0)
*boris_bad_dt_check* value = yes or no
yes/no = enable warning based on |q/m|*|B|*dt_sub
*boris_bad_dt_limit* value = X
X = warning threshold for |q/m|*|B|*dt_sub (X > 0)
...
global efield grid 1 0
global bfield grid 2 0
global boris_subcycles 20
global boris_bad_dt_check yes boris_bad_dt_limit 0.1
global pusher plasma pd mode gca gca_integrator rk2
global pusher plasma pd mode gca gca_integrator rk4
```
...
Assuming your simulation is populated by particles from the background gas, the *fnum* and *nrho* settings can determine how many particles will be present in your simulation, when using the create_particles or "fix emit"_fix_emit_face command variants.
...
See the doc page for the fix field/particle command for the only current fix in OpenEdge which is compatible with the *particle* setting.
...
The *efield* keyword specifies an external electric field for OpenEdge Boris particle pushing.  The *bfield* keyword specifies an external magnetic field.  Both keywords use the same syntax:

```
global efield efstyle fixID Nfreq
global bfield bfstyle fixID Nfreq
```

where efstyle/bfstyle is *none*, *particle*, or *grid*.

The *none* setting disables the field.

The *particle* setting evaluates the field per particle every Nfreq timesteps using a fix efield/particle or fix bfield/particle.  These fixes reference particle-style variables.

The *grid* setting evaluates the field per grid cell every Nfreq timesteps using a fix efield/grid or fix bfield/grid.  These fixes reference grid-style variables or compute sources.  An Nfreq of 0 means the field is computed once at the start of the run and reused for all subsequent timesteps.  An Nfreq of 1 means the field is recomputed every timestep, which is required for time-dependent fields.

All four variants ({efield particle}, {efield grid}, {bfield particle}, {bfield grid}) use the same argument format: fixID followed by Nfreq.

For OpenEdge Boris particle pushing in 3d, additional controls are available:

- *boris_subcycles* splits each particle move timestep into N Boris substeps. - *boris_dump* and *boris_dump_every* control optional Boris debug output of q/m and E/B values. - *boris_bad_dt_check* and *boris_bad_dt_limit* enable a runtime warning if the substep is too large, using |q/m|*|B|*dt_sub > limit.

The *pusher* keyword is the hierarchical OpenEdge interface for Boris, hybrid Boris/GCA, and pure guiding-center motion.  The *plasma* value must identify either a compute plasma/fields command or a fix background command.  Pure *gca* mode always uses GCA when a non-zero magnetic field and charged particle are present.  *hybrid* mode uses *gca_switch* to select between Boris and GCA from the Larmor-radius to magnetic-gradient scale ratio.  *boris_near* can force the Boris path within the specified distance of the configured sheath geometry.

The *gca_integrator* value selects the guiding-center time integrator. *rk4* evaluates the full Littlejohn B-star RHS four times and is the backward-compatible default.  *rk2* uses the explicit midpoint method and evaluates the same full RHS twice.  *simple* uses a reduced one-stage update without the full curvature/curl(b) correction.  In the current CPU implementation, E, B, and their derivatives are sampled once and frozen over the timestep for all stages.  Thus RK2 halves the full-RHS algebra relative to RK4, but not the number of mesh-field queries.

When using *bfield* with the *grid* style, an update frequency of 0 means field values are computed once at run start and then re-used for all subsequent timesteps.  The field is still applied every particle move.

See the doc page for the fix field/grid command for the only current fix in OpenEdge which is compatible with the *grid* setting.

Note that there is a tradeoff between the *particle* and *grid* options.  For the *particle* option the field must be computed every timestep for all particles; the field values are accurately computed at precisely each particle's position but it is an expensive operation.  For the *grid* option the field is only computed once at the beginning of a run or once every Nfreq timesteps.  Even if it is computed every timestep, the number of grid cells is typically much smaller than the number of particles.  However the accuracy of the field applied to each particle is more approximate than for the *particle* option.  This is because the field applied to each particle is the value it has at the center of the particle's grid cell. In OpenEdge 3d Boris pushing with per-grid E/B fields, trilinear interpolation on uniform grids is used when neighbor cell values are available.

---

The *surfs* keyword determines what kind of surface elements OpenEdge uses and how they are distributed across processors.  Possible values are *explicit*, *explicit/distributed*, and *implicit*.  See the Howto 6.13 section of the manual for an explantion of explicit versus implicit surfaces.  The distributed option can be important for models with huge numbers of surface elements.  Each processor stores copies of only the surfaces that overlap grid cells it owns or has ghost copies of.  Implicit surfaces are always distributed.  The *explicit* setting is the default and means each processor stores a copy of all the defined surface elements.  Note that a surface element requires about 100 bytes of storage, so storing a million on a single processor requires about 100 MBytes.
...
Note that the *neigh* style only has an effect (at run time) when the grid is decomposed by the RCB option of the balance or fix balance commands.  If that is not the case, OpenEdge performs the particle communication as if the *all* setting were in place.
...
The keyword defaults are fnum = 1.0, nrho = 1.0, vstream = 0.0 0.0 0.0, temp = 273.15, field = none, efield = none, bfield = none, surfs = explicit, surfgrid = auto, surfmax = 100, splitmax = 10, surftally = auto, gridcut = -1.0, comm/sort = no, comm/style = neigh, weight = cell none, particle/reorder = 0, mem/limit = 0, optmove = no.
