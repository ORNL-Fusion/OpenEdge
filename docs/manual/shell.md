<!-- Adapted from the SPARTA manual (shell.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->
...
shell putenv OpenEdge_DATA=../../data
...
This is means to invoke other commands from your input script.  For example, you can move files around in preparation for the next section of the input script.  Or you can run a program that pre-processes data for input into OpenEdge.  Or you can run a program that post-processes OpenEdge output data.
...
The *cd* cmd executes the Unix "cd" command to change the working directory.  All subsequent OpenEdge commands that read/write files will use the new directory.  All processors execute this command.
...
OpenEdge does not detect errors or print warnings when any of these commands execute.  E.g. if the specified directory does not exist, executing the *cd* command will silently do nothing.
