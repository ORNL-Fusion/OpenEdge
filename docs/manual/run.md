<!-- Adapted from the SPARTA manual (run.txt); Plimpton, Gallis et al., Sandia National Laboratories. OpenEdge extensions marked. -->
...
keyword = {upto} or {start} or {stop} or {pre} or {post} or {every}
  ...
  {every} values = M c1 c2 ...
    ...
    c1,c2,...,cN = one or more OpenEdge commands, each enclosed in quotes
...
The *pre* and *post* keywords can be used to streamline the setup, clean-up, and associated output to the screen that happens before and after a run.  This can be useful if you wish to do many short runs in succession (e.g. OpenEdge is being called as a library which is doing other computations between successive short OpenEdge runs).

By default (pre and post = yes), OpenEdge zeroes statistical counts before every run and initializes other fixes and computes as needed.  And after every run it gathers and prints timings statistics.  If a run is just a continuation of a previous run (i.e. no settings are changed), the initial computation is not necessary.  So if *pre* is specified as "no" then the initial setup is skipped, except for printing statistical info.  Note that if *pre* is set to "no" for the very 1st run OpenEdge performs, then it is overridden, since the initial setup computations must be done.

IMPORTANT NOTE: If your input script changes settings between 2 runs (e.g. adds a fix or compute), then the initial setup must be performed.  OpenEdge does not check for this, but it would be an error to use the {pre no} option in this case.

If *post* is specified as "no", the full timing and statistical output is skipped; only a one-line summary timing is printed.

The *every* keyword provides a means of breaking a OpenEdge run into a series of shorter runs.  Optionally, one or more OpenEdge commands (c1, c2, ..., cN) will be executed in between the short runs.  If used, the *every* keyword must be the last keyword, since it has a variable number of arguments.  Each of the trailing arguments is a single OpenEdge command, and each command should be enclosed in quotes, so that the entire command will be treated as a single argument.  This will also prevent any variables in the command from being evaluated until it is executed multiple times during the run.  Note that if a command itself needs one of its arguments quoted (e.g. the print command), then you can use a combination of single and double quotes, as in the example above or below.

The *every* keyword is a means to avoid listing a long series of runs and interleaving commands in your input script.  For example, a print command could be invoked or a fix could be redefined, e.g. to reset a load balancing parameter.  Or this could be useful for invoking a command you have added to OpenEdge that wraps some other code (e.g. as a library) to perform a computation periodically during a long OpenEdge run.  See "Section 8"_Section_modify of the manual for info about how to add new commands to OpenEdge.  See Section 6.7 of the manual for ideas about how to couple OpenEdge to other codes.
...
If you want OpenEdge to exit early during the middle of a run when a condition is met, use fix halt.
