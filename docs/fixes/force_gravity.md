...
restricted to a particle group via SPARTA's `mixture` machinery on the
...
| `<gx>, <gy>, <gz>` | acceleration vector $[\mathrm{m}/\mathrm{s}^2]$ in **physical (R, Z, φ)** order, not SPARTA slot order. The fix routes through `RZphi_force_to_sparta()` so the same input deck works for axi / 2D-Cart / 3D layouts. |
