...
hard-errors on use. Declare `fix plasma/data` then reference it from
`compute plasma/fields all plasma_data <fix_id> …`.
...
to per-particle FD queries against `fix plasma/data`. Known gap in the
