...
def convert_solps_to_openedge(
    ...
    if (~has_cell_native).any() and len(wall_boundary_edges) > 0:
        ...
        if src_idx.size > 0 and fill_mask.any():
            ...
            distances, j = tree.query(np.column_stack([centroid_r[dst_idx],
                                                        centroid_z[dst_idx]]))
            mesh_cell_idx[dst_idx] = mesh_cell_idx[src_idx[j]]
            tri_source_kind[dst_idx] = 2
            wall_gap_mask[dst_idx] = True
            # EIRENE-style extension: store the distance from each wall-gap
            # triangle centroid to the nearest native (B2-mapped) triangle
            # centroid, so the runtime can apply exponential decay to ne, Te
            # with prescribed decay lengths (Lore NF 2023: lambda_n =
            # lambda_T = 3 cm in the SOL outer-radial direction).
            wall_gap_distance = np.zeros(ntri_eirene, dtype=np.float64)
            wall_gap_distance[dst_idx] = distances
    ...
    # ------------------------------------------------------------------
    # EIRENE-style decay-length extrapolation in the wall-gap region.
    # For each wall-gap triangle, replace the nearest-cell plasma values
    # with exponentially decayed ones based on distance to the nearest
    # native triangle, matching the SOLPS-B2 outer-boundary BC philosophy
    # (Lore NF 2023: lambda_n = lambda_T = 3 cm).
    # We keep ncell_flat fixed and instead APPEND virtual cells, one per
    # wall-gap triangle, with decayed plasma. mesh_cell_idx for those
    # triangles is rerouted to the new appended entries.
    # ------------------------------------------------------------------
    LAMBDA_N = 0.03   # 3 cm density decay length
    LAMBDA_T = 0.03   # 3 cm temperature decay length
    n_wallgap = int(wall_gap_mask.sum())
    if n_wallgap > 0:
        wg_idx = np.flatnonzero(wall_gap_mask)
        wg_dist = wall_gap_distance[wg_idx]
        wg_src_cell = mesh_cell_idx[wg_idx]   # nearest-native-tri's source cell
        decay_n = np.exp(-wg_dist / LAMBDA_N)
        decay_T = np.exp(-wg_dist / LAMBDA_T)

        # Append decayed entries to per-cell plasma arrays.
        # Decay parallel ion velocity with the density length: vi_par scales
        # with the parallel pressure gradient, which scales with n*T; using
        # decay_n as a conservative single-factor decay (Lore SOL: vpar
        # drops with distance from sheath similarly to ne).
        ne_wg  = mesh_ne[wg_src_cell]   * decay_n
        te_wg  = mesh_te[wg_src_cell]   * decay_T
        ti_wg  = mesh_ti[wg_src_cell]   * decay_T
        ni_wg  = mesh_ni[wg_src_cell]   * decay_n
        up_wg  = mesh_upar[wg_src_cell] * decay_n
        mesh_ne   = np.concatenate([mesh_ne,   ne_wg])
        mesh_te   = np.concatenate([mesh_te,   te_wg])
        mesh_ti   = np.concatenate([mesh_ti,   ti_wg])
        mesh_ni   = np.concatenate([mesh_ni,   ni_wg])
        mesh_upar = np.concatenate([mesh_upar, up_wg])

        # Multi-ion arrays: shape (nion, ncell). Append (nion, n_wallgap).
        ions_dens_wg = mesh_ions_dens[:, wg_src_cell] * decay_n[None, :]
        ions_temp_wg = mesh_ions_temp[:, wg_src_cell] * decay_T[None, :]
        ions_upar_wg = mesh_ions_upar[:, wg_src_cell] * decay_n[None, :]
        mesh_ions_dens = np.concatenate([mesh_ions_dens, ions_dens_wg], axis=1)
        mesh_ions_temp = np.concatenate([mesh_ions_temp, ions_temp_wg], axis=1)
        mesh_ions_upar = np.concatenate([mesh_ions_upar, ions_upar_wg], axis=1)

        # Reroute wall-gap triangles to their virtual cells (appended
        # at the end of the per-cell arrays, in wg_idx order).
        new_cell_idx_for_wg = ncell_flat + np.arange(n_wallgap, dtype=np.int32)
        mesh_cell_idx[wg_idx] = new_cell_idx_for_wg
        print(f"EIRENE decay-length extrapolation: {n_wallgap} wall-gap "
              f"triangles assigned virtual cells with exp(-d/3cm) decay; "
              f"max decay factor = {decay_n.min():.3e} at d={wg_dist.max()*100:.1f} cm")
