...
#    else:
    ...
    # --- quick scatter (no binning) ---
#    plt.figure(figsize=(7,4))
#    plt.plot(z, Emag, '.', ms=3)
##    plt.xlabel(axis)
#    plt.ylabel('|E| (V/m)')
##    plt.title(f'|E| vs {axis} (scatter)')
#    plt.tight_layout()
#    plt.show()
#
#    # --- binned 1D average for a clean line plot ---
#    def bin1d(u, v, nbins=100):
#        """Return bin centers and mean(v) per bin (ignoring empty bins)."""
#        umin, umax = np.min(u), np.max(u)
#        edges = np.linspace(umin, umax, nbins+1)
#        idx = np.clip(np.searchsorted(edges, u, side='right')-1, 0, nbins-1)
#
...
#
#    centers, emag_mean = bin1d(u, Emag, nbins=120)
#
#    plt.figure(figsize=(7,4))
#    plt.plot(centers, emag_mean, '-')
##    plt.xlabel(axis)
#    plt.ylabel('⟨|E|⟩ (V/m)')
#    plt.title(f'Binned ⟨|E|⟩ vs {axis}')
#    plt.tight_layout()
#    plt.show()
...
#    t, x, y, z, vx, vy, vz = parse_file("one.dat")
#            
#    fig, ax = plt.subplots(1,1, figsize=(8, 8))  # 2 rows, 5 columns for O to W+
#
