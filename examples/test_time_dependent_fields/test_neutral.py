...
def main():
    ...
    data = parser(f"tmp.forces", 3, 3+3)


    last_timestep = max(data.keys())
    species_labels = ["BR", "BT", "BZ"]
...
#    for last_timestep in [_last_timestep]:
    ...
    # keep your lines that define data and arrays
    xcs, ycs, zcs = data[last_timestep][:3]
    x = np.unique(xcs)
    y = np.unique(ycs)
    z = np.unique(zcs)
    # per-point radius (don't overwrite x/y with R/Z grids)
    xcs = np.asarray(xcs); ycs = np.asarray(ycs); zcs = np.asarray(zcs)
    Bx, By, Bz = data[last_timestep][3:6]
    Bx = np.asarray(Bx); By = np.asarray(By); Bz = np.asarray(Bz)
    r  = np.sqrt(xcs**2 + ycs**2)

    fig, ax = plt.subplots(1, 3, figsize=(22, 8))
    import matplotlib.tri as mtri
    tri = mtri.Triangulation(r, zcs)

    for a, comp, name in zip(ax, [Bx, By, Bz], ["B_x", "B_y", "B_z"]):
        pcm = a.tripcolor(tri, comp, shading='gouraud', cmap='plasma')
        fig.colorbar(pcm, ax=a, label=f'[T]')
        a.set_xlabel('R [m]')
        a.set_ylabel('Z [m]')
        a.set_title(f'{name} in (R,Z) {last_timestep} steps')
        a.axis('scaled')
...
#
#        ax = axes[species_index-3]
#        print(density_grid)
#        print(data[last_timestep][3])
#        exit()
