...
nuE_case1 = epstein_nuE(Ni=1.5746e20, Ti_eV=10.0, rd_m=2.5e-3, rho_d=534.0, alphaE=1.26, A_background=2.0)
nuE_case4 = epstein_nuE(Ni=3.1492e+20, Ti_eV=10.0, rd_m=2.5e-3, rho_d=534.0, alphaE=1.26, A_background=2.0)
...
cases = [
  dict(dump="case.1", dt=dt, g=-9.81, nuE=nuE_case1, u_g=0.0,
       label=rf"v$\parallel$g, $u_g=0$, $\nu_E={nuE_case1:.4f}$ s$^{{-1}}$", v0z_override=+5.0),

  dict(dump="case.2", dt=dt, g=-9.81, nuE=nuE_case1, u_g=0.0,
       label=rf"v$\perp$g, $u_g=0$, $\nu_E={nuE_case1:.4f}$ s$^{{-1}}$", v0z_override=0.0),

  dict(dump="case.3", dt=dt, g=-9.81, nuE=nuE_case1, u_g=1.0,
       label=rf"v$\parallel$g, $u_g=1$ m/s, $\nu_E={nuE_case1:.4f}$ s$^{{-1}}$", v0z_override=+5.0),

  dict(dump="case.4", dt=dt, g=-9.81, nuE=nuE_case4, u_g=1.0,
       label=rf"v$\parallel$g, $u_g=1$ m/s, $\nu_E={nuE_case4:.4f}$ s$^{{-1}}$", v0z_override=+0.0),
]
...
def plot_normalized_collapse(cases, out_png):
    """Plot v_tilde vs t_tilde for all cases with exp(-t_tilde) reference."""
    fig, ax = plt.subplots(1, 1, figsize=(7.0, 5.0), dpi=220)
    ...
    for i, spec in enumerate(cases, 1):
        ...
        ttil = nuE * (t - t[0])
        vtil = (vz - v_inf) / (v0 - v_inf + 1e-300)
        tmax = max(tmax, float(np.max(ttil)))
        ax.plot(ttil, vtil, ".", ms=2.5, alpha=0.85, label=f"T{i}")

    tref = np.linspace(0.0, max(0.05, tmax), 300)
    ax.plot(tref, np.exp(-tref), "-", lw=2.0, color="k", label=r"$e^{-\tilde t}$")

    ax.set_xlabel(r"$\tilde t = \nu_E (t-t_0)$")
    ax.set_ylabel(r"$\tilde v = (v-v_\infty)/(v_0-v_\infty)$")
    ax.set_ylim(-0.02, 1.02)
    ax.set_xlim(0.0, max(0.05, tmax))
...
for i, spec in enumerate(cases, 1):
    ...
    v_end_sim = float(np.median(vz[-max(5,len(vz)//10):]))
    tau = 1.0 / max(nuE, 1e-300)
    t_relax = (t[-1] - t[0]) / tau
    v_inf = ug + g / nuE
    if t_relax < 1.0:
        print(
            f"T{i}: v_end_sim={v_end_sim:.3f}, v_end_ana={v_end_ana:.3f}  |  "
            f"t_end/tau={t_relax:.3f} (far from terminal), tau={tau:.2f} s, v_infty={v_inf:.3f}"
        )
    else:
        print(
            f"T{i}: v_end_sim={v_end_sim:.3f}, v_end_ana={v_end_ana:.3f}  |  "
            f"t_end/tau={t_relax:.3f}, tau={tau:.2f} s, v_infty={v_inf:.3f}"
        )

plot_four_cases_panel(cases, panel_path)
print("Saved:", panel_path)

collapse_path = "Figs/validation_normalized.png"
plot_normalized_collapse(cases, collapse_path)
print("Saved:", collapse_path)
