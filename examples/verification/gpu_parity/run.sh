...
rm -rf out_cpu out_gpu state.restart; mkdir -p out_cpu out_gpu
...
$LAUNCH_GPU $EXE_GPU -k on g 1 -sf kk -pk kokkos react/retry yes -in in.parity -var tag gpu -log log.gpu > screen.gpu 2>&1 || { echo "gpu run failed"; exit 2; }
