...
def parse_wall(path: Path):
    """Return list of (R, Z) line segments for plotting. Cart: file col1=R, col2=Z."""
    pts, segs = [], []
    mode = None
...
def cell_quads_RZ(arr: np.ndarray):
    """Build (R, Z) quad for each AMR cell. SPARTA cart: x=R, y=Z."""
    R_lo, R_hi = arr[:, COL_XLO], arr[:, COL_XHI]
    Z_lo, Z_hi = arr[:, COL_YLO], arr[:, COL_YHI]
...
def main():
    ...
    panel(axes[1], quads, arr[:, COL_WP],    r"$W^{+}$ (singly ionized)",             wall_segs)
