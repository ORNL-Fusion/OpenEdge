...
Open `scripts/analysis.ipynb` before running transport. It checks the raw and
reconstructed plasma, CAD-to-SI conversion, target tiles and voltage waveform,
RustBCA yield, implied radial W source, and the legacy factor-of-ten unit error.
After a run, it adds diagnostics from `output/rfpie_w_density.*.dump`.
...
```sh
time mpirun -np 4 /Users/42d/build_oe/src/spa_mac_mpi -in in.openedge
```

Defaults are `Vdc=-500 V`, `Vrf=0`, and 64 phase samples. Edit
`input/config.json`, rebuild `input/target_face.surf`, and keep `rffreq` in both input
