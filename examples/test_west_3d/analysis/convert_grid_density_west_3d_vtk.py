#!/usr/bin/env python3
"""Convert WEST 3D impurity-density dump to a VTK structured grid (.vts).

Example:
  python3 analysis/convert_grid_density_west_3d_vtk.py \
    --dump output/tmp.grid.density \
    --out output/grid_density_3d.west.vts \
    --smooth 1.0
"""
...
def main():
    ...
    default_out = here / "output" / "grid_density_3d.west.vts"
    default_input = here / "in.west_3d"

    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--dump", default=str(default_dump), help="grid dump file")
    ap.add_argument("--out", default=str(default_out), help="output VTS file")
    ...
    ap.add_argument("--threshold", type=float, default=None, help="Set values below threshold to 0 after smoothing")
    ap.add_argument(
        "--component-labels",
        nargs="*",
    ...
    if args.threshold is not None:
        keep = total_grid >= args.threshold
        total_grid = np.where(keep, total_grid, 0.0)
        component_grids = np.where(keep[None, :, :, :], component_grids, 0.0)
        dominant_component = np.where(keep, dominant_component, 0)
        dominant_fraction = np.where(keep, dominant_fraction, 0.0)
    ...
    for k in range(nz):
        for j in range(ny):
            for i in range(nx):
                pts.SetPoint(idx, float(xu[i]), float(yu[j]), float(zu[k]))
                idx += 1
    sgrid.SetPoints(pts)

    total_vals = total_grid.ravel(order="C").astype(np.float64, copy=False)
    log_total = np.where(total_vals > 0.0, np.log10(total_vals), np.nan)
    sgrid.GetPointData().AddArray(vtk_array(total_vals, "impurity_density"))
    sgrid.GetPointData().AddArray(vtk_array(log_total, "log10_impurity_density"))

    for comp_idx, comp_name in enumerate(component_names):
        comp_vals = component_grids[comp_idx].ravel(order="C").astype(np.float64, copy=False)
        log_comp = np.where(comp_vals > 0.0, np.log10(comp_vals), np.nan)
        sgrid.GetPointData().AddArray(vtk_array(comp_vals, f"impurity_density_{comp_name}"))
        sgrid.GetPointData().AddArray(vtk_array(log_comp, f"log10_impurity_density_{comp_name}"))

    sgrid.GetPointData().AddArray(
        vtk_array(dominant_component.ravel(order="C").astype(np.int32, copy=False), "dominant_component_index", vtk.VTK_INT)
    )
    sgrid.GetPointData().AddArray(
        vtk_array(dominant_fraction.ravel(order="C").astype(np.float64, copy=False), "dominant_component_fraction")
    )
    sgrid.GetPointData().SetActiveScalars("impurity_density")
