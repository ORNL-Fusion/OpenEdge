...
def main():
    ...
    parser.add_argument("--dump", default="case.outer", help="Dump file (Lammps format).")
    parser.add_argument(
        "--quantity",
        choices=["mass", "radius"],
        default="mass",
    ...
    quantity_values = data["pmass"] if args.quantity == "mass" else data["radius"]
    quantity_used = args.quantity
    for pid in ids:
        mask = data["id"] == pid
