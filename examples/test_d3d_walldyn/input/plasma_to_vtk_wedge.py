...
Usage:
    python plasma_to_vtk_wedge.py [--input plasma.h5] [--phi-min 0] [--phi-max 30] [--nphi 1]
"""
...
def main():
    ...
    parser.add_argument("--input", default="input/plasma.h5", help="Path to plasma.h5")
    ...
    for name, data in cell_data.items():
        ...
        arr.SetName(name)
        arr.SetNumberOfTuples(ntri * args.nphi)
        for iphi in range(args.nphi):
            for itri in range(ntri):
                # Map cell index: tris use cell_index from plasma.h5
                # The mesh data is indexed by cell, same order as triangles
                if itri < len(data):
                    arr.SetValue(iphi * ntri + itri, data[itri])
                else:
                    arr.SetValue(iphi * ntri + itri, 0.0)
        ugrid.GetCellData().AddArray(arr)

    # Also add ne from regular grid as a 2D slice (optional)
    # For now just write the mesh-based data
