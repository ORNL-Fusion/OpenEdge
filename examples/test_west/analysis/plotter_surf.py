...
#expects: data[timestep] = [ids] + [col1, col2, col3, ...]


#
#wall_coord = np.zeros(len(vv_r) - 1)
#for i in range(0, len(vv_r) - 1):
#    d_r = vv_r[i + 1] - vv_r[i]
#    d_z = vv_z[i + 1] - vv_z[i]
#    if i == 0:
#        wall_coord[i] = np.sqrt(d_r**2 + d_z**2)
#    else:
#        wall_coord[i] = wall_coord[i - 1] + np.sqrt(d_r**2 + d_z**2)
#wall_coord = np.append(np.array([0]), wall_coord)
#coordinates = list(zip(vv_r, vv_z))
#
#interpolated_values = interp_imp(coordinates)
                
# ------------ example wiring ------------
# data = parse_tmp_surf("tmp.surf", start_col=1, end_col=None)
filename='tmp.all.surf'
data = parse_tmp_surf(filename, start_col=1, end_col=None)

# 1) Pick a timestep (latest)
tmax = max(data.keys())
ids, fcols, xc, yc = unpack_surf_timestep(data, tmax)
q = fcols[0]   # choose which f_save you want, e.g., 0=first

# 2) Your wall vertices (vv_r, vv_z) already defined
data =  np.loadtxt("input/vv_values.csv", unpack=True)
vv_r = data[0]
vv_z = data[1]
s_nodes, s_seg, Rmid, Zmid = wall_arclength(vv_r, vv_z)

# 3) Make a profile along wall arclength from surf centers
s_surf, q_surf = wall_profile_from_surfs(vv_r, vv_z, xc, yc, q)

# 4) (Optional) Resample on the wall vertices or a uniform grid
#    a) at the original wall vertex arclengths:
_, _, q_at_vertices = wall_profile_from_surfs(vv_r, vv_z, xc, yc, q, s_samples=s_nodes)

#    b) at a uniform arclength grid (nice for plotting)
s_uniform = np.linspace(s_nodes[0], s_nodes[-1], 800)
_, _, q_uniform = wall_profile_from_surfs(vv_r, vv_z, xc, yc, q, s_samples=s_uniform)


plt.figure(figsize=(9,4))
#plt.plot(s_surf, q_surf, '.', ms=4, label='surf centers')
plt.plot(s_uniform, q_uniform, '-', lw=1.5) #, label='interp (uniform s)')
plt.xlabel('Wall [m]')
...
#timesteps, ids, vals = align_by_common_ids(data)
##figA, axes = plot_profiles_over_id(timesteps, ids, vals, sample=100)
#names = ["nflux_incident", "mflux", "etot", "press"]
#fig, axes = plot_profiles_over_id(timesteps, ids, vals, names=names, sample=8)
#
##delta_max = compute_convergence_metrics(vals)
### set a tolerance, e.g., relative to last-timestep magnitude
### choose eps as 1e-3 times the 95th percentile of final |values| per quantity
##final_mag = np.nanpercentile(np.abs(vals[-1, :, :]), 95, axis=0)
##eps = float(1e-3 * np.nanmax(final_mag))  # single threshold for simplicity
##figB, axB = plot_convergence(timesteps, delta_max, eps=eps, M=8)
#plt.show()
#
