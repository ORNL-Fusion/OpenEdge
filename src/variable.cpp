...
enum{DONE,ADD,SUBTRACT,MULTIPLY,DIVIDE,CARAT,MODULO,UNARY,
     ...
     SQRT,EXP,LN,LOG,ABS,SIN,COS,TAN,ASIN,ACOS,ATAN,ATAN2,ERF, BESSEL0,
...
/* ----------------------------------------------------------------------
   ...
   customize by adding a function:
     sqrt(),exp(),ln(),log(),abs(),sin(),cos(),tan(),asin(),acos(),atan(),bessel0(),
...
/* ----------------------------------------------------------------------
   ...
   customize by adding a math function:
     ...
     atan2(y,x),random(x,y),normal(x,y),ceil(),floor(),round(),
     ramp(x,y),stagger(x,y),logfreq(x,y,z),stride(x,y,z),
     vdisplace(x,y),swiggle(x,y,z),cwiggle(x,y,z), bessel0(x),
                            ...
                            double *argstack, int &nargstack)
{
  ...
  if (strcmp(word,"sqrt") && strcmp(word,"exp") &&
      ...
      strcmp(word,"vdisplace") && strcmp(word,"bessel0") &&
  ...
  if (tree) {
    ...
    value1 = evaluate(args[0],nullptr);
    if (narg > 1) {
      value2 = evaluate(args[1],nullptr);
      if (narg > 2) {
        for (int i = 2; i < narg; i++)
          values[i-2] = evaluate(args[i],nullptr);
      }
    }
  }
  ...
  // add bessel0 function
  else if (strcmp(word,"bessel0") == 0) {
    if (narg != 1)
      error->all(FLERR,"Invalid math function in variable formula");
    if (tree) newtree->type = BESSEL0;
    // else argstack[nargstack++] = j0(value1);
    else argstack[nargstack++] = I0_fast(value1);
...
int VarReader::read_scalar(char *str)
{
 ...
 double Variable::I0_fast(double x) {
