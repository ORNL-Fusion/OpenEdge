...
void FixChemAdas::end_of_step_no_average()
{
  ...
  for (int icell = 0; icell < nglocal; icell++) {
    ...
    if (cp_plasma) {
      // Per-particle interpolation at actual particle positions
      int ip = cinfo[icell].first;
      while (ip >= 0) {
        PlasmaFileParams pp = cp_plasma->query_plasma_at_point(particles[ip].x);
        const double Te_eV = std::max(pp.temp_e, 1e-6);
        const double ne_m3 = std::max(pp.dens_e, 0.0);
        attempt(&particles[ip], Te_eV, ne_m3);
        ip = next[ip];
      }
    } else {
      // Fallback: per-cell values from grid variable or non-plasma compute
