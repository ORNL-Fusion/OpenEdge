...
FixEthermalGrid::FixEthermalGrid(SPARTA *sparta, int narg, char **arg) :
  Fix(sparta, narg, arg)
{
  if (narg != 5) error->all(FLERR,"Illegal fix ethermal/grid command");
...
FixEthermalGrid::~FixEthermalGrid()
{
  delete [] axstr;
  delete [] aystr;
  delete [] azstr;
...
void FixEthermalGrid::init()
{
  ...
  if (sy.kind == SRC_VAR) {
    ...
    if (ayvar < 0 || !input->variable->grid_style(ayvar))
      error->all(FLERR,"Ey arg for fix ethermal/grid must be grid-style var or c_ID[idx]");
    sy.varid = ayvar;
  }
  if (sz.kind == SRC_VAR) {
    azvar = input->variable->find(azstr);
    if (azvar < 0 || !input->variable->grid_style(azvar))
      error->all(FLERR,"Ez arg for fix ethermal/grid must be grid-style var or c_ID[idx]");
    sz.varid = azvar;
  }

  // bind compute sources
  if (sx.kind == SRC_COMP) bind_compute(sx, "grad_te_r");
  if (sy.kind == SRC_COMP) bind_compute(sy, "grad_te_t");
  if (sz.kind == SRC_COMP) bind_compute(sz, "grad_te_z");
...
void FixEthermalGrid::compute_field()
{
 ...
 ------------------------------------------------------------------------- */
void FixEthermalGrid::parse_src_token(const char *tok, GridSrc &dst, const char *label)
{
...
/* ---------------------------------------------------------------------- 
  ...
  void FixEthermalGrid::bind_compute(GridSrc &S, const char *label)
{
...
void FixEthermalGrid::fill_from_compute(const GridSrc &S, int icol)
{
  Compute *c = modify->compute[S.icompute];
