...
double GeqdskReader::psi_at_point(double R, double Z) const
{
  if (geq.r.empty() || geq.z.empty() || geq.psirz.empty()) return 0.0;
  ...
  auto psi = [&](int ii, int jj) -> double {
    return geq.psirz[jj * nw + ii];
  };

  return (1-t)*(1-u)*psi(i,j) + t*(1-u)*psi(i+1,j)
       + (1-t)*u*psi(i,j+1) + t*u*psi(i+1,j+1);
}

/* ---------------------------------------------------------------------- */

double GeqdskReader::psi_norm_at_point(double R, double Z) const
{
  double psi = psi_at_point(R, Z);
  double dpsi = geq.sibry - geq.simag;
  if (dpsi == 0.0) return 0.0;
  return (psi - geq.simag) / dpsi;
}
