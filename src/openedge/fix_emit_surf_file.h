...
FixStyle(emit/surf/file,FixEmitSurfFile)

#else

#ifndef SPARTA_FIX_EMIT_SURF_FILE_H
#define SPARTA_FIX_EMIT_SURF_FILE_H
...
struct ErodedFluxData{
  std::vector<double> r;   
  std::vector<double> z; 
  std::vector<double> eroded_flux; 
};

// struct PlasmaDataParams {
struct ErodedFluxParams {
  double r;
  double z;
  double eroded_flux;
};
...
class FixEmitSurfFile : public FixEmit {
 public:
  FixEmitSurfFile(class SPARTA *, int, char **);
  ~FixEmitSurfFile();
  void init();

  void grid_changed();

  ErodedFluxData readErodedFlux(const std::string& );
  void initializeErodedFluxData();
  ErodedFluxData  eroded_flux_data;

  void broadcastErodedFluxData(ErodedFluxData& );

  double interpErodedFluxAt(int isurf, double r_val,
                             double z_val,
                             const ErodedFluxData& data,
                             bool clamp_outside) const;
 private:
   mutable std::unordered_map<int, double> flux_cache;  // cache per surface


  char *subsonic_filename;  // new: path to subsonic input file
