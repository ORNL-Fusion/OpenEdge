  ...
  for (i = 0; i < ntask; i++) {
    ...
    // perspecies no
      ...
      for (int m = 0; m < ninsert; m++) {
        ...
        // printf("x before region check: %g %g %g\n",x[0],x[1],x[2]);
        // // get x position to be xpos, ypos, zpos
        // x[0] = 0; //xpos;
        // x[1] = 0.;// ypos;
        // x[2] = 1e-3; //zpos;
        x[0], x[1], x[2] = 0.0117887, -0.00701921, 0.00785963;


        // printf("xpos=%g ypos=%g zpos=%g\n",xpos,ypos,zpos);
        if (region && !region->match(x)) continue;
        ...
        // v[2] = vnmag*normal[2] + vamag*atan[2] + vbmag*btan[2];
          ...
          // double mass_kg = particle->species[isp].mass;
          ...
          v[0] = vscale[isp] * sin(rad_phi) * cos(rad_theta);
          v[1] = vscale[isp] * sin(rad_phi) * sin(rad_theta);
          v[2] = vscale[isp] * cos(rad_phi);
