...
enum{NRHO,TEMP_THERMAL,TEMP_ROT,TEMP_VIB,VX,VY,VZ,PRESS,SPECIES, GAMMA};
...
void FixEmitFaceFile::read_file(char *file, char *section)
{
  ...
  for (i = 0; i < mesh.nvalues; i++) {
    ...
    else if (strcmp(word,"gamma") == 0) mesh.which[i] = GAMMA;
    else {
...
int FixEmitFaceFile::interpolate(int icell)
{
  ...
  if (dimension == 2) {
    for (m = 0; m < mesh.nvalues; m++) {
      ...
      else if (mesh.which[m] == GAMMA)
        tasks[ntask].gamma = linear_interpolation(xc[0],m,plo,phi);
      else {
        anyfrac = 1;
        isp = -mesh.which[m] - 1;
  ...
  double Gamma_face = tasks[ntask].gamma;  
  double ntargetsp;
  for (isp = 0; isp < nspecies; isp++) {
    // ntargetsp = frac_user *
      // mol_inflow(indot,tasks[ntask].vscale[isp],tasks[ntask].fraction[isp]);
    // double gamma = tasks[ntask].gamma;               // [m^-2 s^-1]
    double Gamma_sp   = frac_user * Gamma_face * tasks[ntask].fraction[isp];
    // printf("Gamma_sp for species %d: %g\n", isp, Gamma_sp);
    ntargetsp  = Gamma_sp * area * dt / fnum;
