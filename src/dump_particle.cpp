...
enum{ID,TYPE,PROC,CELLID,X,Y,Z,XS,YS,ZS,VX,VY,VZ,KE,EROT,EVIB,
     CUSTOM,COMPUTE,FIX,VARIABLE, RADIUS, TEMP};
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        for (i = 0; i < nlocal; i++) dchoose[i] = particles[i].radius;
        ptr = dchoose;
        nstride = 1;
      } // TEMP
      else if (thresh_array[ithresh] == TEMP) {
        for (i = 0; i < nlocal; i++) dchoose[i] = particles[i].temp;
        ptr = dchoose;
        nstride = 1;
...
int DumpParticle::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      pack_choice[i] = &DumpParticle::pack_radius;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"temp") == 0) {
      pack_choice[i] = &DumpParticle::pack_temp;
      vtype[i] = DOUBLE;

      // temp, and radius are custom attributes
...
void DumpParticle::pack_radius(int n)
{
  ...
  for (int i = 0; i < nchoose; i++) {
    buf[n] = particles[clist[i]].radius;
    n += size_one;
  }
}
/* ---------------------------------------------------------------------- */
void DumpParticle::pack_temp(int n)
{
  Particle::OnePart *particles = particle->particles;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = particles[clist[i]].temp;
    n += size_one;
  }
}