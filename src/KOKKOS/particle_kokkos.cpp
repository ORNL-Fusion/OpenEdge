  ...
  if (space == Device) {
    ...
    if (mask & CUSTOM_MASK) {
      if (ncustom) {
        if (ncustom_ivec)
          for (int i = 0; i < ncustom_ivec; i++)
            if (k_eivec.view_host()[i].k_view.d_view.data())
              k_eivec.view_host()[i].k_view.sync_device();

        if (ncustom_iarray)
          for (int i = 0; i < ncustom_iarray; i++)
            if (k_eiarray.view_host()[i].k_view.d_view.data())
              k_eiarray.view_host()[i].k_view.sync_device();

        if (ncustom_dvec)
          for (int i = 0; i < ncustom_dvec; i++)
            if (k_edvec.view_host()[i].k_view.d_view.data())
              k_edvec.view_host()[i].k_view.sync_device();

        if (ncustom_darray)
          for (int i = 0; i < ncustom_darray; i++)
            if (k_edarray.view_host()[i].k_view.d_view.data())
    ...
    if (mask & CUSTOM_MASK) {
      if (ncustom_ivec)
        for (int i = 0; i < ncustom_ivec; i++)
          if (k_eivec.view_host()[i].k_view.h_view.data())
            k_eivec.view_host()[i].k_view.sync_host();

      if (ncustom_iarray)
        for (int i = 0; i < ncustom_iarray; i++)
          if (k_eiarray.view_host()[i].k_view.h_view.data())
            k_eiarray.view_host()[i].k_view.sync_host();

      if (ncustom_dvec)
        for (int i = 0; i < ncustom_dvec; i++)
          if (k_edvec.view_host()[i].k_view.h_view.data())
            k_edvec.view_host()[i].k_view.sync_host();

      if (ncustom_darray)
        for (int i = 0; i < ncustom_darray; i++)
          if (k_edarray.view_host()[i].k_view.h_view.data())
