...
FixIthermalGrid::FixIthermalGrid(SPARTA *sparta, int narg, char **arg) :
  Fix(sparta, narg, arg)
{
  if (narg != 5) error->all(FLERR,"Illegal fix ithermal/grid command");
...
FixIthermalGrid::~FixIthermalGrid()
{
  delete [] axstr;
  delete [] aystr;
  delete [] azstr;
...
void FixIthermalGrid::init()
{
  // check if all variables exist and are grid-style vars

  // parse each token: either grid variable name or c_ID[idx]
  if (axstr) parse_src_token(axstr, sx, "grad_te_r");
  if (aystr) parse_src_token(aystr, sy, "grad_te_t");
  if (azstr) parse_src_token(azstr, sz, "grad_te_z");

  // resolve legacy variables
  if (sx.kind == SRC_VAR) {
    axvar = input->variable->find(axstr);
    if (axvar < 0 || !input->variable->grid_style(axvar))
      error->all(FLERR,"Ex arg for fix ethermal/grid must be grid-style var or c_ID[idx]");
    sx.varid = axvar;
  }
  if (sy.kind == SRC_VAR) {
    ayvar = input->variable->find(aystr);
    if (ayvar < 0 || !input->variable->grid_style(ayvar))
      error->all(FLERR,"Ey arg for fix ithermal/grid must be grid-style var or c_ID[idx]");
    sy.varid = ayvar;
  }
  if (sz.kind == SRC_VAR) {
    azvar = input->variable->find(azstr);
    if (azvar < 0 || !input->variable->grid_style(azvar))
      error->all(FLERR,"Ez arg for fix ithermal/grid must be grid-style var or c_ID[idx]");
    sz.varid = azvar;
  }

  // bind compute sources
  if (sx.kind == SRC_COMP) bind_compute(sx, "grad_ti_r");
  if (sy.kind == SRC_COMP) bind_compute(sy, "grad_ti_t");
  if (sz.kind == SRC_COMP) bind_compute(sz, "grad_ti_z");
...
void FixIthermalGrid::compute_field()
{
 ...
 ------------------------------------------------------------------------- */
void FixIthermalGrid::parse_src_token(const char *tok, GridSrc &dst, const char *label)
{
  ...
  if (strncmp(tok,"c_",2)==0) {
    ...
    if (!lb || tok[strlen(tok)-1] != ']') {
      char msg[160];
      snprintf(msg,sizeof(msg),"fix ithermal/grid: bad %s token (use c_ID[idx])",label);
      error->all(FLERR,msg);
    }
    int idlen = lb - name;
    ...
    if (dst.col <= 0) {
      char msg[160];
      snprintf(msg,sizeof(msg),"fix ithermal/grid: %s column must be >=1",label);
      error->all(FLERR,msg);
    }
  } else {
    dst.kind = SRC_VAR;
...
/* ---------------------------------------------------------------------- 
  ...
  void FixIthermalGrid::bind_compute(GridSrc &S, const char *label)
{
  S.icompute = modify->find_compute(S.cid);
  if (S.icompute < 0) {
    char msg[160];
    snprintf(msg,sizeof(msg),"fix ithermal/grid: compute ID for %s not found",label);
    error->all(FLERR,msg);
  }
  Compute *c = modify->compute[S.icompute];
  if (c->per_grid_flag == 0) {
    char msg[160];
    snprintf(msg,sizeof(msg),"fix ithermal/grid: compute for %s is not per-grid",label);
    error->all(FLERR,msg);
  }
  if (c->size_per_grid_cols == 0) {
    char msg[160];
    snprintf(msg,sizeof(msg),"fix ithermal/grid: compute for %s has no per-grid array",label);
    error->all(FLERR,msg);
  }
  if (S.col < 1 || S.col > c->size_per_grid_cols) {
    char msg[160];
    snprintf(msg,sizeof(msg),"fix ithermal/grid: column for %s out of range",label);
    error->all(FLERR,msg);
  }
} 


void FixIthermalGrid::fill_from_compute(const GridSrc &S, int icol)
{
  Compute *c = modify->compute[S.icompute];
  ...
  if (ok <= 0 || !arr) error->all(FLERR,"fix ithermal/grid: compute has no per-grid data");
