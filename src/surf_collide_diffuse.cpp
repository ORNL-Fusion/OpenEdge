...
void SurfCollideDiffuse::diffuse(Particle::OnePart *p, double *norm)
{
  ...
  // tangent12 are both unit vectors
    ...
    const double R_pos   = std::max(p->x[0], 1e-8);
    const double Z_pos   = p->x[1];
    double temp_i_wall  = update->interpolatePlasma_RZ_clamped(R_pos, Z_pos, update->plasma_data).temp_i;
    tsurf = random_energy_thompson(update->target_material_binding_energy, temp_i_wall) * update->ev2kelvin;
    if (tsurf <= 0.0) {
      tsurf = update->target_material_binding_energy * update->ev2kelvin;
    }
