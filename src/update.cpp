                            ...
                            double charge, double mass)
{
  ...
  int sgroupbit = ~0;
  const int dim_local = domain->dimension;
  ...
  for (int isub = 0; isub < nsub; isub++) {
    ...
    // // print x y z E B for first particle at first subcycle for debugging
    // printf("Boris3D subcycle %d/%d: x=(%g,%g,%g) E=(%g,%g,%g) B=(%g,%g,%g)\n",
    //        isub+1, nsub, xcur[0], xcur[1], xcur[2], E[0], E[1], E[2], B[0], B[1], B[2]);
